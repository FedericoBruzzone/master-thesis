...
import neverlang.core.typesystem.defaults.MultipleTypeTypeBinder;
...
public class TypingEnvironment<IDENTIFIER> {
  ...
  private TypingEnvironment(
      @NotNull HashMap<IDENTIFIER, EntryTypeBinder> map,
      @NotNull Class<? extends EntryTypeBinder> typeBinderClass) {
    this.map = map;
  ...
  public TypingEnvironment<IDENTIFIER> bindTypeToIdentifier(
      IDENTIFIER variable, SymbolTableEntry anyType) {
    try {
      var typeBinder =
          map.getOrDefault(variable, this.typeBinderClass.getConstructor().newInstance());
  ...
  public static class Builder<IDENTIFIER> {
    @NotNull
    private Class<? extends EntryTypeBinder> typeBinderClass = MultipleTypeTypeBinder.class;
