...
@SignatureAnnotation(SignatureEnum.FUNCTION)
@TypeLangAnnotation(keyword = "function", kind = TypeSystemKind.SIGNATURE)
    ...
    if (symbolTableEntry.type() instanceof AbstractTypeFunction typeFunction) {
      ...
      typeFunction
          ...
          .forEach(e -> {
            ...
            var signatureParamType = getPositionalParameter(paramTypeEntry.position());
      ...
      var res = typeFunction
          .streamSymbolTableEntries()
          .filter(e -> e.details() instanceof ReturnEntry)
          .findFirst();
      if (res.isEmpty()) {
        throw new UndefinedReturnTypeException(
            "No return type found for function " + symbolTableEntry.entryType().token(),
            symbolTableEntry.location());
      }
      return res.get();
    } else {
