package neverlang.lsp.clients.nvim;
...
public class NvimTemplateGenerator implements TemplateGenerator {
    ...
    private void createPathAndFolder() {
        try {
            // Create a folder called NvimClient
            var unzipFolder = this.unzipPath + "/" + name.toLowerCase() + "-nvim-client-0.0.1";
    ...
    public TemplateGenerator generateLSP() throws IOException {
        var lspFile = this.getClass().getClassLoader().getResourceAsStream("lsp.lua");
        var lspContent = new String(lspFile.readAllBytes(), StandardCharsets.UTF_8);
        var lspString = MessageFormat.format(lspContent, name, jarPath);
    ...
    private String escapeCharactersNvim(String s) {
    ...
    public void generateSyntaxHighlighter(Map<SyntaxElement, List<Symbol>> m) throws Exception {
        ...
        if (toListOfString(m.get(SyntaxElement.MULTILINECOMMENT)).size() > 1 ) {
            multilineCommentStart = escapeCharactersNvim(toListOfString(m.get(SyntaxElement.MULTILINECOMMENT)).get(0));
            multilineCommentEnd = escapeCharactersNvim(toListOfString(m.get(SyntaxElement.MULTILINECOMMENT)).get(1));
        } else {
