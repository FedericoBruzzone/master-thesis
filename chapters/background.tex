...
Instead, we discuss \texttt{Neverlang} (Sect.~\ref{subsec:neverlang}) in particular detail, since most of the research discussed in this dissertation will use \texttt{Neverlang} as a running example.
...
\subsection{Type Systems}\label{subsec:TypeSystems}
...
\subsection{Theoretical Aspects}\label{subsec:TheoreticalAspects}
...
The first group of inference rules in Martin-L\"of's type theory are the \textit{formation rules}. These rules define how types are formed and how elements are formed in a given context. The formation rules are used to define the types of variables and functions in the code.
\begin{prooftree}
...
\hfill \break
\hfill \break
The second group of inference rules in Martin-L\"of's type theory are the \textit{introduction rules}. These rules define how elements are introduced in a given context. The introduction rules are used to define the elements of variables and functions in the code.
\begin{prooftree}
...
\hfill \break
\hfill \break
The third group of inference rules in Martin-L\"of's type theory are the \textit{elimination rules}. These rules define how elements are eliminated in a given context. The elimination rules are used to define the elimination of variables and functions in the code.
...
\hfill \break
\hfill \break
The fourth group of inference rules in Martin-L\"of's type theory are the \textit{equality rules}. These rules define how types and elements are equal in a given context. The equality rules are used to define the equality of types and elements in the code.
...
In listing~\ref{lst:typeinference} we show an example of type inference in Rust and OCaml. In both examples, the type inference algorithm is used to deduce the types of the variables \texttt{z} in the code. In case of Rust code, the type of \texttt{z} is inferred to be \texttt{i32} because the types of \texttt{x} and \texttt{y} are known to be \texttt{i32}. In case of OCaml code, the type of \texttt{z} is inferred to be \texttt{int} because the types of \texttt{x} and \texttt{y} are known to be \texttt{int}.
